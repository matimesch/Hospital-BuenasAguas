    ...
    if (req.session.loggedUser) {
        if (req.session.loggedUser.profile == "admin") {
            functions.getMedicos(medicosList => {
                ...
                res.render("pending", { layout: "main2Admin", user: req.session.loggedUser, medicosList: medicosList });
            })
        }
        else {
            res.redirect("/login");
...
pacientesRouter.get("/home", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/home", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
...
pacientesRouter.get("/turnos", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/turnos/turnos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
        ...
        functions.getMedicos(medicosList => {
            if (req.body.name) {
                ...
                if (req.session.loggedUser) {
                    ...
                    res.render("paciente/turnos/reserva", { layout: "main2", user: req.session.loggedUser, medicosList: medicosList, reservaMSG });
                } else {
                    res.redirect("/login");
                }
            } else {
                ...
                res.render("paciente/turnos/turnos", { layout: "main2", user: req.session.loggedUser, turnoNoEmptyMSG });
            }
    ...
    if (req.session.loggedUser) {
        ...
        functions.saveTurno(req.body, req.session.loggedUser.email, turnoAgregado => {
            ...
            if (turnoAgregado) {
                ...
                res.render("paciente/turnos/turnos", { layout: "main2", user: req.session.loggedUser, turnoMessage });
...
pacientesRouter.get("/misturnos", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/turnos/misturnos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
...
pacientesRouter.get("/medicosFAV", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/medicos/medicos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
...
pacientesRouter.get("/mismedicos", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/medicos/mismedicos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
...
pacientesRouter.get("/medicamentosFAV", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/medicamentos/medicamentos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
...
pacientesRouter.get("/mismedicamentos", (req, res) => {
    if (req.session.loggedUser) {
        res.render("paciente/medicamentos", { layout: "main2", user: req.session.loggedUser });
    } else {
        res.redirect("/login");
    }
});
